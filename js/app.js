    ...
    (() => {
        ...
        const burger = () => {
            ...
            if (burgerBtn && burgerCloseBtn && menu && overlay) {
                ...
                burgerBtn.addEventListener("click", (() => {
                    ...
                    if (menu.classList.contains("initial-hide")) menu.classList.remove("initial-hide");
                ...
                document.addEventListener("click", (event => {
                    if (!menu.contains(event.target) && !burgerBtn.contains(event.target)) closeMenu();
                }));
        ...
        function initLoadMore() {
            ...
            if (teamList && loadMoreBtn) {
                ...
                for (let i = cardsToShowInitially; i < cards.length; i++) cards[i].style.display = "none";
                loadMoreBtn.addEventListener("click", (function() {
                    for (let i = cardsToShowInitially; i < cards.length; i++) cards[i].style.display = cards[i].style.display === "none" ? "" : "none";
                    const buttonText = loadMoreBtn.textContent.trim();
                    loadMoreBtn.textContent = buttonText === "Більше спеціалістів" ? "Сховати" : "Більше спеціалістів";
                }));
            }
        }
        function isWebp() {
        ...
        const openProgram = () => {
            ...
            openBtns.forEach((button => {
                button.addEventListener("click", (() => {
                    ...
                    if (lessonContainer) {
                        const lessonContent = lessonContainer.querySelector(".program-section__lesson-content");
                        const lessonNumber = lessonContainer.querySelector(".lesson-block-number");
        ...
        const myModal = () => {
            ...
            function handleClickOutsideModal(event) {
                if (!modalBody.contains(event.target) && event.target !== connectButtons[0]) closeModal();
            }
                ...
                form.addEventListener("submit", (async event => {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            event.target.reset();
                            closeModal();
                        } else throw new Error("Помилка при відправці форми");
                    } catch (error) {
        ...
        function headerFixed() {
            if (elementsNodeList_header) {
                ...
                if (scrollY >= elementsNodeList_header.clientHeight - 30) elementsNodeList_header.classList.add("fixed");
                window.addEventListener("scroll", (() => {
                    let currentScrollPosition = scrollY || document.documentElement.scrollTop;
                    try {
                        if (!elementsNodeList_header.classList.contains("active")) if (scrollY >= elementsNodeList_header.clientHeight - 30) elementsNodeList_header.classList.add("fixed"); else elementsNodeList_header.classList.remove("fixed");
